...
func (m ChatMessage) MarshalJSON() ([]byte, error) {
	...
	if len(m.MultiContent) > 0 {
		msg := struct {
			...
			Content      string            `json:"-"`
			MultiContent []llm.ContentPart `json:"content,omitempty"`
	...
	msg := struct {
		...
		MultiContent []llm.ContentPart `json:"-"`
		Name         string            `json:"name,omitempty"`
		ToolCalls    []ToolCall        `json:"tool_calls,omitempty"`
...
func isSingleTextContent(parts []llm.ContentPart) (string, bool) {
	if len(parts) != 1 {
		return "", false
	}

	tc, isText := parts[0].(llm.TextContent)
...
func (m *ChatMessage) UnmarshalJSON(data []byte) error {
	msg := struct {
		Role         string            `json:"role"`
		Content      string            `json:"content"`
		MultiContent []llm.ContentPart `json:"-"` // not expected in response
		Name         string            `json:"name,omitempty"`
		ToolCalls    []ToolCall        `json:"tool_calls,omitempty"`
...
func parseStreamingChatResponse(ctx context.Context, r *http.Response, payload *ChatRequest) (*ChatCompletionResponse,
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if err := json.NewDecoder(bytes.NewReader([]byte(data))).Decode(&streamPayload); err != nil {
